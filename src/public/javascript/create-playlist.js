...
async function uploadPlaylist(event) {
...
function updateMusicList() {
    const elements = playlistMusic.map(({ id, title }) => `
    <li>
...
async function addListItem() {
    ...
    try {
        ...
        playlistMusic.push({ id: data.music_id, title: data.music_name });
        addMusicInput.value = '';
        updateMusicList();
    } catch (error) {
        console.error(error);
...
function deleteListItem(id) {
    playlistMusic = playlistMusic.filter(({ id: listItemId }) => listItemId != id);
    updateMusicList();
}
