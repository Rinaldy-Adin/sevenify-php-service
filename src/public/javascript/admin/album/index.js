async function updateDataList(adios, searchResults) {
    const elements = await Promise.all(searchResults.map(async (row, idx) => {
        ...
        let coverSrc = '';
        try {
            const coverResp = await adios.get(`/api/album-cover/${row['album_id']}`, {}, true);
        ...
        rowElements.push(
            `<td>
                <div class="data-action">
                    <a href="/admin/album/update/${row['album_id']}">
                        <svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 -960 960 960" width="20">
                            <path d="M200-200h56l345-345-56-56-345 345v56Zm572-403L602-771l56-56q23-23 56.5-23t56.5 23l56 56q23 23 24 55.5T829-660l-57 57Zm-58 59L290-120H120v-170l424-424 170 170Zm-141-29-28-28 56 56-28-28Z" />
                        </svg>
                    </a>
                    <div class="clickable" onclick="deleteAlbum(${row['album_id']})">
                        <svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 -960 960 960" width="20">
...
async function deleteAlbum(id) {
    ...
    try {
        const resp = await adios.delete('/api/admin/album/' + id);
...
function changePage(page) {
    loadDataPage('/api/admin/album', page, updateDataList);
}
