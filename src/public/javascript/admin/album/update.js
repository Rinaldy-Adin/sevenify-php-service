...
async function uploadAlbum(event, albumId) {
    ...
    try {
        await confirmCancelPopup('Update Album', "Are you sure you want to update?");
    } catch (error) {
        return;
    }

    try {
        const resp = await adios.postFormData('/api/admin/album/' + albumId, event.target, additionalData);
